...
class DuedatecalculatorValidationException implements Exception {
  final List<String> errors;

  DuedatecalculatorValidationException(this.errors);

  @override
  String toString() => 'DuedatecalculatorValidationException: ${errors.join("; ")}';
}
...
/// Due Date Calculator API client.
///
/// For more information, visit: https://apiverve.com/marketplace/duedatecalculator?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [lastPeriod] (required) - First day of last menstrual period (YYYY-MM-DD) [format: date]
class DuedatecalculatorClient {
  final String apiKey;
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'last_period': ValidationRule(type: 'string', required: true, format: 'date'),
  };

  DuedatecalculatorClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/duedatecalculator',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [DuedatecalculatorValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw DuedatecalculatorValidationException(errors);
    }
  }

  /// Execute a request to the Due Date Calculator API.
  ///
  /// Parameters are validated before sending the request.
  Future<DuedatecalculatorResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw DuedatecalculatorException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return DuedatecalculatorResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw DuedatecalculatorException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw DuedatecalculatorException('Resource not found');
      } else {
        throw DuedatecalculatorException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is DuedatecalculatorException) rethrow;
      throw DuedatecalculatorException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Due Date Calculator API client.
class DuedatecalculatorException implements Exception {
  final String message;

  DuedatecalculatorException(this.message);

  @override
  String toString() => 'DuedatecalculatorException: $message';
}
